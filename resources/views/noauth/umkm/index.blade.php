...
@section('title', 'Statistik Data UMKM')
...
<div class="container py-5">
    ...
    <div class="row mb-5 justify-content-center">
        <div class="col-lg-3 col-md-6 mb-4">
            <div class="stat-card">
                <p>Total UMKM</p>
                <div class="stat-number text-primary">{{ number_format($totalUmkm) }}</div>
            </div>
        </div>
        <div class="col-lg-3 col-md-6 mb-4">
            <div class="stat-card" style="border-left-color: #28a745;"> <p>Total Karyawan</p>
                <div class="stat-number text-success">{{ number_format($totalKaryawan) }}</div>
            </div>
        </div>
    ...
    <div class="row mb-5">
        <div class="col-lg-4 col-md-6 mb-4">
            <div class="chart-card">
                <h6>Pertumbuhan Bulanan (Akumulasi {{ $tahunSekarang }})</h6>
                <div class="chart-container">
                ...
                <div class="card-body pt-0 px-3">
                    <div class="d-flex flex-wrap gap-2 mb-3" id="bulanFilter">
                        @foreach(range(1,12) as $bulan)
                            <button type="button"
                                class="btn btn-sm btn-outline-primary bulan-btn {{ request('bulan','all')==$bulan?'active':'' }}"
                                data-bulan="{{ $bulan }}">
                                {{ \Carbon\Carbon::create()->month($bulan)->translatedFormat('M') }}
                            </button>
                        @endforeach
                        <button type="button"
                            class="btn btn-sm btn-outline-primary bulan-btn {{ request('bulan','all')=='all'?'active':'' }}"
    ...
    <div class="chart-card mt-4 p-4" id="tabelUmkm">
        ...
        <form method="GET" action="{{ route('noauth.umkm.index') }}" id="searchForm" class="mb-3 d-flex">
            ...
            <input type="text" name="nama" value="{{ request('nama') }}" class="form-control me-2" placeholder="Cari UMKM berdasarkan nama..." id="searchInput">
            <button type="submit" class="btn btn-primary"><i class="bi bi-search"></i> Cari</button>
        </form>
        
        <!-- <div id="umkmTable">
            {{-- @include('noauth.umkm.table') akan tampil di sini --}}
        </div> -->
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        ...
        function updateTable(extraParams = {}) {
            ...
            $.get("{{ route('noauth.umkm.index') }}", activeFilters, function (data) {
